...
printf("error");
        ...
        printf("\nThe ServerD finishes sending its neighbor information to the client with TCP port number %d and IP address %s",ntohs(cli_addr.sin_port),inet_ntoa(cli_addr.sin_addr));
        printf("\nFor this connection wiht the Client, the Server D has TCP port number %d and IP address %s",ntohs(serv_addr.sin_port),inet_ntoa(serv_addr.sin_addr));
        printf("\nThe server D has received the network topology from the client with UDP port number %d and IP address %s",ntohs(cli_addr1.sin_port),inet_ntoa(cli_addr1.sin_addr));
       

        buf[recv_len]='\0';
        printf("%s\n" ,buf);
        
        printf("\nFor this connection with the Client, The Server D has UDP port number %d IP address %s\n",ntohs(serv_addr1.sin_port),inet_ntoa(serv_addr1.sin_addr));
